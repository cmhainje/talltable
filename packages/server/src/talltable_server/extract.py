...
def download_cutout_pixels(ra,dec,cutout_size_deg=0.05,wvbounds=None):
    ...
    if wvbounds is None:
        query = (
                 talltable.PixelQuery(web=False)
                 .disc(ra, dec, radius)
...
def process_target(name, ra, dec, pm_ra, pm_dec, ref_epoch, args):
    all_results: List[ExtractionResult] = []

    if image_tab is None or psf_cubes is None:
        raise RuntimeError("need to load image_tab and psf_cubes")
    ...
    try:
        ...
        try:
            ...
            sleep(5)
            raise RuntimeError(f"could not process {name}")
...
def write_results(name, all_results, args):
    # ------------------------------------------------------------------
...
def main(argv=None):
    global image_tab, psf_cubes, sapm_images
    ...
    for name, ra, dec, pm_ra, pm_dec, ref_epoch in targets:
        if os.path.isfile(os.path.join(args.results_dir, f"{name}_spherex_photometry.csv")):
            print("  Already extracted this object, skipping to the next one.")
            continue

        try:
            results = process_target(name, ra, dec, pm_ra, pm_dec, ref_epoch, args)
            write_results(name, results, args)
        except RuntimeError:
            failed.append(name)
